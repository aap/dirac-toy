...
To get an intuition for the boosts
start in the default position and drag the z-slider.
Notice how one hand approaches the origin while the other approaches infinity.
The origin stays between the hands but changes the relative position between them.

Physically the starting position corresponds to massive spin-up matter at rest,
the boost in the z-direction gives it a velocity in the z-direction,
the magnitude being determined by the ratio between left and right hand.
Massless matter is described by just a single hand.
Obviously this is a fundamentally different setup
in which there is no variable velocity,
so lightspeed is the only available choice.

Boosting in the x and y directions from the starting position
slides the hands around in the x/y planes.
...
but to represent these needs a 4R spinor.

---

The full Dirac spinor is a pair of left and right Weyl spinors, represented as quaternions:
`ψ = [ψ_L, ψ_R]`.

On this we act with Cl(1,3) in the standard chiral representation:
```
1    = [ 1   ]              γ_0123 = [ i     ]
       [   1 ]                       [    -i ]
